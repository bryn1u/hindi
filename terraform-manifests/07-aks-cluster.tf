...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  name                = "${azurerm_resource_group.k8s-rg-single.name}-cluster"
  location            = azurerm_resource_group.k8s-rg-single.location
  resource_group_name = azurerm_resource_group.k8s-rg-single.name
  dns_prefix          = "${azurerm_resource_group.k8s-rg-single.name}-cluster"
  kubernetes_version  = data.azurerm_kubernetes_service_versions.latest.latest_version
  node_resource_group = "${azurerm_resource_group.k8s-rg-single.name}-nrg"

  default_node_pool {
    name = "systempool"
    #    node_count = 1
    vm_size              = "Standard_D2_v2"
    orchestrator_version = data.azurerm_kubernetes_service_versions.latest.latest_version
    zones                = [3]
  ...
  azure_active_directory_role_based_access_control {
    ...
    admin_group_object_ids = [azuread_group.aks_administrator.id]
  }
